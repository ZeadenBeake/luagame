...
-- Each helper receives (actorList, partyChars) and returns an ordered list of actor ids.
-- actorList: the array passed to startEncounter's 'actors' key
-- partyChars: eng.state.party.characters

local function isPlayerActor(actor, partyChars)
...
function M.playersFirst(actorList, partyChars)
  ...
  for _, a in ipairs(actorList) do
    if isPlayerActor(a, partyChars) then players[#players + 1] = a.id
